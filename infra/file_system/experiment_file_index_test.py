...
def test_list_subjects(parser,mocker) -> None:
    ...
    file_system_mock.listdir.side_effect = lambda path:tree.get(path,[])
    file_system_mock.isdir.side_effect = lambda path:path in tree
...
def test_list_sessions(parser: FileNameParser,mocker):
    tree = {
        "\\data":["なし","常時"],
        "\\data\\なし":["なし"],
        "\\data\\なし\\なし":["subject1"],
    ...
    assert sessions == {Condition(CoolingMode.ALWAYS,Position.CAROTID),Condition(CoolingMode.ALWAYS,Position.NECK)}

def test_scan_with_mocked_parser(parser: FileNameParser,mocker) -> None:
    # Arrange
    tree = {
    ...
    assert SessionFileCandidates([
        ExperimentFile("\\data\\ALWAYS\\CAROTID\\subject1\\file1.csv",FileKind.SSQ_BEFORE,datetime(2024, 1, 1, 12, 0, 0)),
        ExperimentFile("\\data\\ALWAYS\\CAROTID\\subject1\\file2.csv",FileKind.FMS,datetime(2024, 1, 1, 12, 10, 0))])
